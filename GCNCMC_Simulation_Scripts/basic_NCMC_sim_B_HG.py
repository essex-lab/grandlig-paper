...
parser.add_argument("--hxml", help="Input PDB file of solvated protein/complex")
parser.add_argument("--lig", help="PDB file of ligand to add as ghosts")
parser.add_argument("--lxml", help="XML file containing parameters for the ligands")
...
parser.add_argument(
    "--sph_atoms",
    help="Atom names to define sphere betweeen. Must be part of resn MGO",
    type=str,
    nargs="+",
    default=None,
)
...
# GCMC Sphere
s_atoms = args.sph_atoms
ref_atoms = []
for i in range(len(s_atoms)):
    ref_atoms.append(
        {"name": f"{s_atoms[i]}", "resname": "MGO", "resid": "1"}
    )
