...
parser.add_argument(
    "-lims",
    "--lims",
    help="Input Insertion Work files",
    type=float,
    nargs="+",
    default=None,
)
...
# Filter out non_binders from frags


filtered_frags = {k: v for k, v in frags_dict.items() if k not in non_binders}
print(filtered_frags)
filtered_data = np.asarray(list(filtered_frags.values()))
filtered_frags = list(filtered_frags.keys())

f_x_dg = filtered_data[:, 0]
f_x_error = filtered_data[:, 1]
f_y_dg = filtered_data[:, 2]
f_y_error = filtered_data[:, 3]
...
# Getting all the stats using the filtered sets
x_data_stats = f_x_dg
y_data_stats = f_y_dg
...
rmse_ci, mae_ci, r_squared_ci, pearsons_ci, kendall_ci, spearman_ci = boot_stats(f_x_dg, f_y_dg, f_x_error, f_y_error)
...
for i, frag in enumerate(frags):
    if frag in non_binders:
        if not labelled:
            ax2.scatter(x_dg[i], y_dg[i], marker='o', s=75, linestyle='None', c='b', zorder=2, label='Nonbinders')
            labelled = True
        else:
            ax2.scatter(x_dg[i], y_dg[i], marker='o', s=75, linestyle='None', c='b', zorder=2)
        ax2.errorbar(x_dg[i], y_dg[i], yerr=y_error[i], fmt='none', c='black', zorder=1, capsize=2)
        ax2.errorbar(x_dg[i], y_dg[i], xerr= abs(x_dg[i] - ax_max), fmt='_', c='grey', zorder=1, capsize=5, capthick=2, ecolor='grey', elinewidth=1, markeredgewidth=2, xlolims=True)
    else:
        ax2.scatter(x_dg[i], y_dg[i], marker='o', s=75, linestyle='None', c='r', zorder=2)
        ax2.errorbar(x_dg[i], y_dg[i], xerr=x_error[i], yerr=y_error[i], fmt='none', c='black', zorder=1, capsize=2)
...
# ax2.set_facecolor('#EDEDED')
# plt.legend(loc='upper left') #, facecolor='#EDEDED')
