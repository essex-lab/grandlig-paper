...
def calc_mu_ex(id, n_repeats=4):
    WORK_DIR = os.getcwd()
    ligand = id
    #print(WORK_DIR)
    #print(ligand)
    repeats = range(1, n_repeats+1)
    mu_folder = f"{id}/Mu_Ex"
    # Load first np array to get info from
    
    u_ = np.load(f"{mu_folder}/repeat_1/lambda_0/U_matrix_0.npy")
    ...
    for repeat in repeats:
        if not os.path.isfile(f"{mu_folder}/repeat_{repeat}/lambda_0/U_matrix_0.npy"):
            continue
        U_kln = np.zeros((n_lambdas, n_lambdas, n_samples))
        for i in range(0, n_lambdas):  # Load in all the data to the big U_kln array
            npy_array = f'{mu_folder}/repeat_{repeat}/lambda_{i}/U_matrix_{i}.npy'
...
df["Mu_Ex"] = mu_exs
df["Mu_Ex_err"] = mu_ex_errs

df.to_csv("ligands_with_mu.txt", index=False)
