...
# Input Files
pdb = PDBFile('sol-ghosts2.pdb')  # Because this is a continuation, load in the file with ghosts - only needed for topology
...
if args.run == 2:
    ...
    n_moves_lig, n_accepted_lig, n_moves_hoh, n_accepted_hoh = read_moves('hoh-{}.log'.format(args.run -1))
    wat_ghosts = grand.utils.read_ghosts_from_file('hoh-ghosts-{}.txt'.format(args.run-1))[-1]
    ghosts = grand.utils.read_ghosts_from_file('ncmc-ghost-ligs-{}.txt'.format(args.run-1))[-1]
...
params, custom_nb_force, ele_except, vdw_except = grand.utils.create_custom_forces(system, pdb.topology, ['L02', 'HOH'])


###### WATER #######
# Create GCMC sampler for water
water_sampler = grand.samplers.NonequilibriumGCMCSystemSampler(system=system,
                                                               topology=pdb.topology,
                                                               temperature=298*kelvin,
                                                               integrator=integrator,
                                                               nPertSteps=n_pert_wat,
                                                               nPropStepsPerPert=n_prop_wat, timeStep=dt,
                                                               # Make sure the two parameters below are correct for the ligand concentration
                                                               excessChemicalPotential=wat_mu_ex, standardVolume=wat_vol,
...
ncmc_mover = grand.samplers.NonequilibriumGCMCSystemSampler(system=system, topology=pdb.topology,
                                                            temperature=temperature,
                                                            integrator=integrator,
                                                            excessChemicalPotential=mu_ex,
                                                            adams=None, standardVolume=volume_per_lig,
                                                            nPertSteps=n_pert, nPropStepsPerPert=n_prop, timeStep=dt,
                                                            boxVectors=np.array(pdb.topology.getPeriodicBoxVectors()), 
                                                            resname='L02', createCustomForces=False,
                                                            ghostFile='ncmc-ghost-ligs-{}.txt'.format(args.run),
                                                            log='bulk_conc_0.1-{}.log'.format(args.run),
                                                            rst='checkpoint-{}.rst7'.format(args.run),
                                                            dcd='bulkconc_raw-{}.dcd'.format(args.run), overwrite=True)
