...
plot_path = os.path.join(SCRIPT_DIR, "../../../Free_Energy_Scripts")
...
def calc_mu_ex(id, n_repeats=3):
    ...
    u_file = f"{ligand}/Complex/repeat_1/0/U_matrix_0.npy"
    if not os.path.isfile(u_file):
        raise Exception("HERE")
    
    u_ = np.load(f"{ligand}/Complex/repeat_1/0/U_matrix_0.npy")
    n_lambdas, _, n_samples = u_.shape
    n_samples = 1000
    print(n_lambdas, n_samples)
    ...
    for repeat in repeats:
        if not os.path.isfile(f"{ligand}/Complex/repeat_{repeat}/0/U_matrix_0.npy"):
            continue
        U_kln = np.zeros((n_lambdas, n_lambdas, n_samples))
        for i in range(0, n_lambdas):  # Load in all the data to the big U_kln array
            npy_array = f'{ligand}/Complex/repeat_{repeat}/{i}/U_matrix_{i}.npy'
            if not os.path.isfile(npy_array):
                pass
                #raise Exception(f"Cannot find file: {npy_array}")
            u_kln_ = np.load(npy_array)[:,:,:n_samples]
            U_kln += u_kln_
        np.save(f"{ligand}/Complex/repeat_{repeat}/Combined_Ukln.npy", U_kln)
...
df = pd.read_csv(args.df, delimiter=args.delim, converters={"Ligand": str})
